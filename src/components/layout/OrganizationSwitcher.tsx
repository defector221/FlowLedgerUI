  ...
  const content = (
    <div className="flex min-w-0 items-center gap-2">
      ...
      <div className={sidebar ? 'min-w-0 flex-1 text-left' : 'hidden min-w-0 text-left md:block'}>
        <p
          className={
            sidebar ? 'truncate text-sm font-medium text-white' : 'truncate text-sm font-medium text-slate-900'
          }
        >
      ...
      {multiple && (
        <ChevronDown className={sidebar ? 'size-4 shrink-0 text-slate-400' : 'size-4 shrink-0 text-slate-500'} />
      )}
    </div>
  )
