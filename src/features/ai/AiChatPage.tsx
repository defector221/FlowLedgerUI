...
export function AiChatPage() {
  ...
  return (
    <div className="flex h-[calc(100dvh-7.5rem)] min-h-[32rem] flex-col gap-5">
      <PageHeader
        title="AI Assistant"
        subtitle="Ask about inventory, sales, GST, and cash — answers use live ERP data via tools. Advisory only."
        actions={
            ...
            <span className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-2.5 py-1 font-medium text-slate-600 shadow-sm">
              <span
                className={cn(
                  ...
                  health.isSuccess ? 'bg-teal-500' : 'bg-slate-300',
                )}
              />
      ...
      <div className="grid min-h-0 flex-1 gap-4 lg:grid-cols-[minmax(15rem,17.5rem)_1fr]">
            ...
            {conversations.data?.map((c) => (
              ...
              >
                ...
                <p className="mt-0.5 flex items-center justify-between gap-2 text-[11px] text-slate-400">
                  <span className="truncate">{c.agentType?.replace(/_/g, ' ') || 'Assistant'}</span>
        ...
        <section className="flex min-h-0 flex-col overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-[var(--shadow-soft)]">
          <div className="flex items-center justify-between gap-3 border-b border-slate-100 bg-gradient-to-r from-teal-50/60 via-white to-sky-50/40 px-5 py-3.5">
            <div className="min-w-0">
              ...
              <p className="mt-0.5 text-xs text-slate-500">
                {activeConversation?.agentType
                  ? `${activeConversation.agentType.replace(/_/g, ' ')} · ERP tools + knowledge`
                  : 'Domain agents route tools for live numbers; RAG for policies.'}
              </p>
            </div>
          </div>
            ...
            {showEmpty ? (
              <div className="mx-auto flex max-w-xl flex-col items-center px-2 py-8 text-center sm:py-12">
                ...
                <p className="mt-2 max-w-md text-sm leading-relaxed text-slate-500">
                  Ask operational questions. I pull from your books and inventory through secure
                  read-only tools — I never post journals or stock.
                </p>
            ...
            {rows.map((m) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {!user ? (
                      <div className="mb-2 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-wide text-teal-700">
                    ...
                    {m.createdAt ? (
                      <p
                        className={cn(
                          'mt-2 text-[10px]',
                          user ? 'text-teal-100/80' : 'text-slate-400',
                        )}
                      >
          ...
          <div className="border-t border-slate-100 bg-slate-50/40 p-3 sm:p-4">
            ...
            <p className="mt-2 px-1 text-[11px] leading-relaxed text-slate-400">
              Advisory only. AI does not create invoices, payments, or stock adjustments without your
              confirmation in the ERP screens.
            </p>
          </div>
        </section>
      </div>
